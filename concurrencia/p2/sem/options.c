...
static void usage(int i)
{
    printf(
        ...
        "  -w n  --customer_wait=<n>: waiting time for customers to arrive (customer number * n)\n"
        "  -s n  --max_seats=<n>: number of chairs in the waiting room\n"
        "  -h, --help: this message\n\n"
    );
    exit(i);
}
...
int handle_options(int argc, char **argv, struct options *opt)
{
    while (1) {
        ...
        c = getopt_long (argc, argv, "ht:c:b:w:s:",
        ...
        case 'w':
            if (!get_int(optarg, &opt->customer_wait)
                || opt->customers <= 0) {
                printf("'%s': is not a valid integer\n",
                       optarg);
                usage(-3);
        ...
        case 's':
            if (!get_int(optarg, &opt->max_seats)
                || opt->customers <= 0) {
                printf("'%s': is not a valid integer\n",
                       optarg);
                usage(-3);
