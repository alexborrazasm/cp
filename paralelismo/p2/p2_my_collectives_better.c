    ...
    MPI_Datatype datatype, MPI_Op op, int root, MPI_Comm comm)
{
    ...
    int *send = (int *)sendbuf, *recv = (int *)recvbuf;
...
{
    int rank, v_rank, numprocs, result;
    ...
    v_rank = (rank - root + numprocs) % numprocs; // Calculate it for any root

    if (v_rank != 0) // not root 
    {
    ...
    int dest, real_dest;
    for (int i = 1; (dest = v_rank | i) < numprocs; i <<= 1) // *2
    {
        if (v_rank < i)
        {
            real_dest = (dest + root) % numprocs; // virtual rank to rank
            result = MPI_Send(buffer, count, datatype, real_dest, 2034, comm);
            if (result != MPI_SUCCESS) return result;
        }
    }
