...
void *print(void *ptr)
{
    ...
    while (finish(args->should_terminate, args->print_mutex))
    {
        int i;
...
void start_threads(struct options opt)
{
    ...
    shouldTerminate = malloc(sizeof(bool));
    printMutex = malloc(sizeof(pthread_mutex_t));

    if (!printThread || !printArgs || !shouldTerminate || !printMutex) {
        printf("Not enough memory\n");
        exit(1);
    }
    ...
    *shouldTerminate = false;
    ...
    if (pthread_mutex_init(printMutex, NULL))
    {
        printf("Can not initiate mutex\n");
        exit(1);
    }
    ...
    printArgs->should_terminate = shouldTerminate;
    printArgs->print_mutex = printMutex;
    ...
    *shouldTerminate = true;
    ...
    if (pthread_mutex_destroy(printMutex))
    {
        printf("Can not destroy mutex\n");
        exit(1);
    }
